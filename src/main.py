...
def create_dataframe(artists: list[Artist]) -> pd.DataFrame:
    ...
    for i, artist in enumerate(artists):
        print(f'\rprocessing artist {i+1}/{n}...', end='')
...
def get_relevant_songs_for_artist(artist: Artist, all_artist_ids: list[str]) -> list[Song]:
    ...
    # split album_ids into chunks of 20, request and return responses as one whole
    album_ids = [album_ids[i:i+20] for i in range(0, len(album_ids), 20)]
    albums: list[dict] = []
    for ids_chunk in album_ids:
        albums += sp_albums(ids_chunk)['albums']
